document.addEventListener("DOMContentLoaded", function () {
  ...
  // Elements
  const form = document.getElementById("doctorateForm");
  const inputs = {
  ...
  const validators = {
    ...
    bachelorsDegreeMajor: () => {
      const valid = inputs.bachelorsDegreeMajor.value.trim().length > 0;
      toggleError(errors.bachelorsDegreeMajor, valid);
      return valid;
    },
    mastersDegreeMajor: () => {
      const valid = inputs.mastersDegreeMajor.value.trim().length > 0;
      toggleError(errors.mastersDegreeMajor, valid);
      return valid;
    },
  ...
  inputs.bachelorsDegreeMajor.addEventListener(
    "input",
    validators.bachelorsDegreeMajor
  );
  inputs.mastersDegreeMajor.addEventListener(
    "input",
    validators.mastersDegreeMajor
  );
